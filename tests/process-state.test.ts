...
describe("proxy process-state ownership", () => {
  ...
  test("keeps lifecycle records in the selected runtime directory", () => {
    const runtimeDir = join(testDir, "slot-a");
    setRuntimeDir(runtimeDir);

    expect(getRuntimeDir()).toBe(runtimeDir);
    expect(getPidPath()).toBe(join(runtimeDir, "ocx.pid"));
    expect(getRuntimePortPath()).toBe(join(runtimeDir, "runtime-port.json"));
    writePid(1234);
    expect(existsSync(join(runtimeDir, "ocx.pid"))).toBe(true);
    expect(existsSync(join(testDir, "ocx.pid"))).toBe(false);

    setRuntimeDir(undefined);
    expect(getRuntimeDir()).toBeUndefined();
    expect(getPidPath()).toBe(join(testDir, "ocx.pid"));
  });
