...
import { expandUserPath, getConfigDir, hardenConfigDir } from "./paths";

let runtimeDirOverride: string | undefined;

/** Lifecycle records are slot-private; durable state remains under OPENCODEX_HOME. */
...
function processStateDir(): string {
  return runtimeDirOverride ?? getConfigDir();
}

export function getPidPath(): string {
  return join(processStateDir(), "ocx.pid");
}

export function getRuntimePortPath(): string {
  return join(processStateDir(), "runtime-port.json");
}
...
function ensureProcessStateDir(): void {
  ...
  else if (dir === getConfigDir()) hardenConfigDir();
}
